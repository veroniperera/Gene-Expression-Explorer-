...
async function init() {
  dataset = await d3.json("visualization_data.json");
  populateDropdown();
  document.getElementById("plotBtn").addEventListener("click", drawLineChart);
  drawLineChart(); // initial draw
}

function populateDropdown() {
  const select = document.getElementById("geneSelect");
  const knownGenes = dataset.filter(d => geneNames[d.GeneID]);
  knownGenes.forEach((d, i) => {
    const opt = document.createElement("option");
    opt.value = d.GeneID;
    opt.text = geneNames[d.GeneID];
    if (i < 3) opt.selected = true; // default selection
    select.appendChild(opt);
  });
}

function getGeneSeries(geneRow) {
  return samples.map(sample => ({ sample, value: geneRow[sample] }));
}

function drawLineChart() {
  d3.select("#wrapper").selectAll("*").remove(); // clear previous chart

  const select = document.getElementById("geneSelect");
  const selectedIds = Array.from(select.selectedOptions).map(o => Number(o.value));
  const genesToPlot = dataset.filter(d => selectedIds.includes(d.GeneID));

  if (genesToPlot.length === 0) return;
  ...
  let dimensions = {
    ...
    height: 400,
    margin: { top: 40, right: 140, bottom: 50, left: 70 }
  };
